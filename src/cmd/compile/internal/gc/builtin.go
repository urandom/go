...
var runtimeDecls = [...]struct {
	...
	{"closechanwith", funcTag, 89},
	{"writeBarrier", varTag, 91},
	{"typedmemmove", funcTag, 92},
	{"typedmemclr", funcTag, 93},
	{"typedslicecopy", funcTag, 94},
	{"selectnbsend", funcTag, 95},
	{"selectnbrecv", funcTag, 96},
	{"selectnbrecv2", funcTag, 98},
...
func runtimeTypes() []*types.Type {
	...
	typs[89] = functype(nil, []*Node{anonfield(typs[2]), anonfield(typs[3])}, nil)
	typs[90] = types.NewArray(typs[0], 3)
	typs[91] = tostruct([]*Node{namedfield("enabled", typs[6]), namedfield("pad", typs[90]), namedfield("needed", typs[6]), namedfield("cgo", typs[6]), namedfield("alignme", typs[24])})
	typs[92] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[3]), anonfield(typs[3])}, nil)
	typs[93] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[3])}, nil)
	typs[94] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[3]), anonfield(typs[15]), anonfield(typs[3]), anonfield(typs[15])}, []*Node{anonfield(typs[15])})
	typs[95] = functype(nil, []*Node{anonfield(typs[87]), anonfield(typs[3])}, []*Node{anonfield(typs[6])})
	typs[96] = functype(nil, []*Node{anonfield(typs[3]), anonfield(typs[84])}, []*Node{anonfield(typs[6])})
	typs[97] = types.NewPtr(typs[6])
	typs[98] = functype(nil, []*Node{anonfield(typs[3]), anonfield(typs[97]), anonfield(typs[84])}, []*Node{anonfield(typs[6])})
	typs[99] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[1]), anonfield(typs[15])}, []*Node{anonfield(typs[15]), anonfield(typs[6])})
